use crate::{structures::cmd_data::PrefixMap, ConnectionPool};
...
pub async fn add_new_guild(pool: &PgPool, guild_id: GuildId, is_new: bool) -> CommandResult {
    ...
    if delete_data.exists.unwrap() {
        sqlx::query!(
            "DELETE FROM delete_time_store WHERE guild_id = $1",
            guild_id.0 as i64
        )
        .execute(pool)
        .await?;
        ...
        sqlx::query!(
            "INSERT INTO guild_info VALUES($1, null, null, null) ON CONFLICT DO NOTHING",
            guild_id.0 as i64
        )
        .execute(pool)
        .await?;
...
pub async fn delete_leftover_reactions(pool: &PgPool, message_id: MessageId) -> CommandResult {
    let check = sqlx::query!(
        "SELECT EXISTS(SELECT 1 FROM reaction_roles WHERE message_id = $1)",
        message_id.0 as i64
    )
    .fetch_one(pool)
    .await?;
