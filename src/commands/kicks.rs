...
async fn kick(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if kick_user_id == msg.author.id {
        msg.channel_id.say(ctx, RoyalError::SelfError("kick")).await?;
    ...
    if permissions_helper::check_moderator(ctx, msg, Some(kick_user_id)).await? {
        msg.channel_id.say(ctx, 
            RoyalError::PermissionError(PermissionType::Mention("kick", "administrator/moderator"))).await?;
    ...
    match guild_id.kick_with_reason(ctx, kick_user_id, &reason).await {
        ...
        Err(e) => {
            msg.channel_id.say(ctx, RoyalError::UnsuccessfulError("Kick")).await?;
